...
import {
  ...
  getWalletConnected,
  mintZIPNFT
} from "../utils/etherMint";
import { pinFileToIPFS } from "../utils/pinata.js";

const EtherMinter = (props) => {
  ...
  const onMintPressed = async () => {
    ...
    if (uploadSuccess) {
      const { success, status } = await mintZIPNFT(url, name, description);
      setStatus(status);
      if (success) {
        setName("");
        setDescription("");
  ...
  return (
    <div className="Minter">
      ...
      <br></br>
      <h1 id="title">File Mint on Ethereum</h1>
      <p>
      ...
      <form>
        ...
        <input 
          type="file"
          accept=".zip,.rar,.mp3,.mp4,.avi, .jpg, .png, .gif"
